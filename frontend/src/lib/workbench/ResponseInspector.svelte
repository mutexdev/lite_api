...
<div class="response-inspector">
  {#if selectedTab === 'response'}
    ...
    {:else}
      <pre class="response-body" bind:this={bodyElement} data-match-index={matches[matchIndex] ?? -1}>{#each markedParts(safeDisplay) as part}<span>{#if part.match}<mark class:current-match={part.index === matchIndex}>{part.text}</mark>{:else}{part.text}{/if}</span>{/each}</pre>
    {/if}
    ...
    {#if (request.examples ?? []).length > 0}
      <section class="response-compare" aria-label="Response comparison">
        <label>Compare with <select aria-label="Compare response with" bind:value={compareId}>{#each compareOptions as option}<option value={option.id}>{option.name}</option>{/each}</select></label>
        {#if compareId !== 'current'}
          ...
          <section class="compare-section" aria-label="Header changes"><h4>Headers</h4><table><thead><tr><th>Header</th><th>Current</th><th>Selected</th><th>Change</th></tr></thead><tbody>{#each headerComparison.filter((row) => showUnchanged || row.change !== 'unchanged') as row}<tr><td>{row.name}</td><td>{row.current}</td><td>{row.selected}</td><td><span class={`compare-badge ${row.change}`}>{row.change}</span></td></tr>{/each}</tbody></table></section>
          ...
          <div class="response-diff">{#each diff.rows.filter((row) => showUnchanged || row.changed) as row}<div class:changed={row.changed}><code>{row.left}</code><code>{row.right}</code></div>{/each}</div>
  ...
  {:else if selectedTab === 'headers'}
    ...
    {#if filteredHeaders.length === 0}<div class="empty-state">No headers match this search.</div>{:else}<table><tbody>{#each filteredHeaders as [name, value]}<tr><td>{name}</td><td>{value}</td></tr>{/each}</tbody></table>{/if}
  {:else if selectedTab === 'metadata' || selectedTab === 'trailers'}
    ...
    {#if rows.length === 0}<div class="empty-state">No {selectedTab}</div>{:else}<table><tbody>{#each rows as row}<tr><td>{row.name}</td><td>{row.value}</td></tr>{/each}</tbody></table>{/if}
  {:else if selectedTab === 'timeline'}
    ...
    {:else}
      <div class="timeline">{#each filteredTimeline as entry, index (`${entry.id}:${entry.phase ?? ''}:${entry.at ?? ''}:${index}`)}<article class="timeline-entry"><button type="button" aria-expanded={expandedTimelineID === entry.id} on:click={() => expandedTimelineID = expandedTimelineID === entry.id ? '' : entry.id}><span>{entry.status || entry.statusText || '-'}</span><strong>{entry.method || entry.kind}</strong><span>{entry.url || entry.message}</span><small>{timelinePhase(entry)} · {entry.duration || 0} ms</small></button>{#if expandedTimelineID === entry.id}<div class="timeline-detail">{#if entry.sourceFile}<small>Source: {entry.sourceFile}</small>{/if}{#if entry.source}<small>Kind/source: {entry.kind} · {entry.source}</small>{/if}<code>{entry.error || entry.payload || entry.message}</code>{#if (entry.metadata?.length ?? 0) > 0}<table data-testid="timeline-grpc-metadata"><tbody>{#each entry.metadata ?? [] as row}<tr><td>{row.name}</td><td>{row.value}</td></tr>{/each}</tbody></table>{/if}{#if (entry.trailers?.length ?? 0) > 0}<table data-testid="timeline-grpc-trailers"><tbody>{#each entry.trailers ?? [] as row}<tr><td>{row.name}</td><td>{row.value}</td></tr>{/each}</tbody></table>{/if}</div>{/if}</article>{/each}</div>
    {/if}
  {:else if selectedTab === 'console'}
    {#if scriptLogs.length === 0}<div class="empty-state">No console output</div>{:else}<div class="console-log-list">{#each scriptLogs as log}<div class={`console-row ${log.level || 'log'}`}><span>{log.level || 'log'}</span><code>{log.message}</code></div>{/each}</div>{/if}
  {:else if selectedTab === 'tests'}
    <div class="results">{#each response?.assertions ?? [] as assertion}<div class:passed={assertion.passed} class:failed={!assertion.passed}>{assertion.expression} {assertion.message}</div>{/each}{#each response?.testResults ?? [] as test}<div class:passed={test.passed} class:failed={!test.passed}>{test.name} {test.message}</div>{/each}</div>
  {/if}
</div>
