...
func goTestNamesInPackage(t *testing.T) map[string]bool {
	...
	// it -- scanning only this package made the ledger report those as missing
	// the moment internal/interp took the dynamic-variable tests.
	//
	// The root is resolved rather than written as ".": `go test` sets the
	// working directory to the package directory, so "." stopped meaning the
	// module the moment this package moved under internal/, and the walk
	// quietly narrowed to one directory.
	err := filepath.WalkDir(repoRoot(t), func(path string, entry fs.DirEntry, err error) error {
		if err != nil {
			return err
		}
		if entry.IsDir() {
...
func TestPostmanParityDocumentExistsAndCoversEveryRow(t *testing.T) {
	data, err := os.ReadFile(repoPath(t, "POSTMAN-PARITY.md"))
	if err != nil {
	...
	if _, err := os.Stat(repoPath(t, "PARITY.md")); err != nil {
...
func TestPostmanParityDocumentCitesRealFixtures(t *testing.T) {
	...
	for _, match := range matches {
		if _, err := os.Stat(filepath.Join(repoRoot(t), filepath.FromSlash(match[1]))); err != nil {
