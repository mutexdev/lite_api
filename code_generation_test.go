...
func codegenFixture(bodyMode string) ResponseExample {
...
func allCodegenIDs() []string {
	ids := make([]string, 0, len(codegenLanguages))
...
// TestCodegenTargetsAreDispatchable. The picker is built from codegenLanguages,
...
func TestCodegenTargetsAreDispatchable(t *testing.T) {
	...
	for _, target := range app.CodeGenerationTargets() {
		t.Run(target.ID, func(t *testing.T) {
			...
			if _, err := generateResponseExampleCode(example, target.ID); err != nil {
	...
	for _, target := range codegenLanguages {
		for _, alias := range target.Aliases {
			if _, err := generateResponseExampleCode(example, alias); err != nil {
